...
4. **Synthesis.** A final agent folds the validated drafts into one coherent response. The synthesizer receives a constraint block listing all locked facts from the ledger. Output is cleaned of any leaked `<reasoning>` tags, prompt instruction echoes, and `<answer>` wrappers.

5. **Post-Synthesis Validation.** The synthesizer's output is re-scanned by `_post_synthesis_validate()` before reaching the user. This catches any hallucinations introduced during the rewriting phase — anachronisms, fact contradictions, or setting violations. If the synthesized text fails validation, the system automatically falls back to the highest-scored raw draft from the drafting phase, which already passed symbolic validation.

*Source: `_post_synthesis_validate()` in `src/swarm.py`, line 1130. Fallback: best validated draft by score.*
...
graph TD
    ...
    N --> P{"Post-Synthesis<br/>Validation"}
    C --> P
    P -->|"Pass"| O["Final Response"]
    P -->|"Fail"| Q["Fallback to<br/>Best Draft"]
    ...
    style P fill:#8b4513,stroke:#cd853f,color:#fff
```
...
The PyQt6 interface provides real-time visibility into the reasoning pipeline:

- **Live Pipeline Monitor.** A scrolling status panel embedded in the chat area streams real-time progress: which agents are active, which drafts passed or failed symbolic validation, and what the ledger locked. This appears during processing and collapses after the response is delivered. *Source: `show_thinking()`, `update_agent_status()`, `update_ledger_status()` in `src/ui/widgets/chat_area.py`.*

- **State Ledger Viewer.** The right sidebar displays every locked fact for the active thread, updated after each message. Protected predicates (setting, genre, era, etc.) are marked with a lock icon. *Source: `set_ledger_state()` in `src/ui/widgets/sidebar_right.py`.*

- **Thread Management.** Create, switch, and delete individual threads via the left sidebar. Right-click context menu supports per-thread deletion, which also wipes the thread's facts from the state ledger. *Source: `_on_delete_thread()` in `src/ui/controller.py`.*
...
**Synthesizer Trust Gap (Partially Closed).** The synthesizer can introduce hallucinations while rewriting safe drafts. Pipeline step 5 (Post-Synthesis Validation) catches these by re-scanning the output against the state ledger and falling back to the best validated draft on failure. Additionally, `_clean_synthesis()` strips leaked `<reasoning>` tags, `<answer>` wrappers, and prompt instruction echoes. This closes the trust gap for most cases, though adversarial prompt injection at the synthesis stage remains a theoretical risk.

**Domain Tuning Required.** While the system includes a nuance layer (`_FIGURATIVE_PATTERNS`) to distinguish metaphors ("hit by a truck") from literal anachronisms, this logic is currently tuned for narrative and roleplay contexts. The medieval era blocklist (104 terms) provides strong coverage for fantasy and historical fiction. Deploying this system in medical, legal, or scientific domains requires rewriting `src/constraints.py` with domain-appropriate blocklists and validation rules.
